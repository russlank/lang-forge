...
func renderParserRules(prefix string, table *parse.Table, actionIDs map[string]string) string {
	...
	for _, rule := range table.Rules {
		b.WriteString(ruleSourceComment(rule, "c", "// "))
	...
	for _, rule := range table.Rules {
		...
		b.WriteString(indentComment(ruleSourceComment(rule, "c", "// "), "    "))
		...
		for _, sym := range symbols {
			...
			if action.Kind == parse.ActionShift {
				...
				if rule, ok := ruleByID(table.Rules, action.Rule); ok {
					entries = append(entries, indentComment(ruleSourceComment(rule, "c", "// "), "    "))
				}
			}
