...
func renderParser(namespace string, source string, project *spec.Spec, table *parse.Table, tokens []string, tokenIDs map[string]string, actions []SemanticAction, actionManifest action.Manifest) string {
	...
	b.WriteString("/// <summary>Thrown by strict parse APIs when syntax diagnostics were produced.</summary>\n")
	...
	b.WriteString("    /// <summary>Recognizes a materialized lexeme collection.</summary>\n")
	b.WriteString("    public static void Parse(IReadOnlyList<Lexeme> lexemes) => new Parser().ParseInput(lexemes);\n")
	b.WriteString("    /// <summary>Recognizes lexemes pulled lazily from a scanner or lexeme source.</summary>\n")
	b.WriteString("    public static void Parse(ILexemeSource source) => new Parser().ParseLexemeSource(source);\n")
	b.WriteString("    /// <summary>Parses a materialized lexeme collection and returns the final semantic value.</summary>\n")
	b.WriteString("    public static object? ParseValue(IReadOnlyList<Lexeme> lexemes) => new Parser().ParseValueInput(lexemes);\n")
	b.WriteString("    /// <summary>Parses lexemes pulled lazily from a scanner or lexeme source and returns the final semantic value.</summary>\n")
	b.WriteString("    public static object? ParseValue(ILexemeSource source) => new Parser().ParseValueLexemeSource(source);\n")
	b.WriteString("    /// <summary>Parses a materialized lexeme collection with grammar-directed recovery.</summary>\n")
	b.WriteString("    public static ParseResult ParseRecovering(IReadOnlyList<Lexeme> lexemes) => new Parser().ParseRecoveringInput(lexemes);\n")
	b.WriteString("    /// <summary>Parses lexemes pulled lazily from a scanner or lexeme source with grammar-directed recovery.</summary>\n")
	b.WriteString("    public static ParseResult ParseRecovering(ILexemeSource source) => new Parser().ParseRecoveringLexemeSource(source);\n")
	b.WriteString("    /// <summary>Parses a materialized lexeme collection with grammar-directed recovery and an explicit reducer.</summary>\n")
	b.WriteString("    public static ParseResult ParseRecovering(IReadOnlyList<Lexeme> lexemes, IReducer reducer) => new Parser(reducer).ParseRecoveringInput(lexemes);\n")
	b.WriteString("    /// <summary>Parses lexemes pulled lazily from a scanner or lexeme source with grammar-directed recovery and an explicit reducer.</summary>\n")
	b.WriteString("    public static ParseResult ParseRecovering(ILexemeSource source, IReducer reducer) => new Parser(reducer).ParseRecoveringLexemeSource(source);\n")
	b.WriteString("    /// <summary>Parses a materialized lexeme collection with an explicit reducer.</summary>\n")
	b.WriteString("    public static object? ParseWithReducer(IReadOnlyList<Lexeme> lexemes, IReducer reducer) => new Parser(reducer).ParseValueInput(lexemes);\n")
	b.WriteString("    /// <summary>Parses lexemes pulled lazily from a scanner or lexeme source with an explicit reducer.</summary>\n")
	b.WriteString("    public static object? ParseWithReducer(ILexemeSource source, IReducer reducer) => new Parser(reducer).ParseValueLexemeSource(source);\n\n")
	b.WriteString("    /// <summary>Named wrapper for Parse(ILexemeSource). Prefer the overload in handwritten C# code.</summary>\n")
	b.WriteString("    public static void ParseFromLexemeSource(ILexemeSource source) => Parse(source);\n")
	b.WriteString("    /// <summary>Named wrapper for ParseValue(ILexemeSource). Prefer the overload in handwritten C# code.</summary>\n")
	b.WriteString("    public static object? ParseValueFromLexemeSource(ILexemeSource source) => ParseValue(source);\n")
	b.WriteString("    /// <summary>Named wrapper for ParseRecovering(ILexemeSource). Prefer the overload in handwritten C# code.</summary>\n")
	b.WriteString("    public static ParseResult ParseRecoveringFromLexemeSource(ILexemeSource source) => ParseRecovering(source);\n")
	b.WriteString("    /// <summary>Named wrapper for ParseWithReducer(ILexemeSource, IReducer). Prefer the overload in handwritten C# code.</summary>\n")
	b.WriteString("    public static object? ParseWithReducerFromLexemeSource(ILexemeSource source, IReducer reducer) => ParseWithReducer(source, reducer);\n\n")
	b.WriteString("    /// <summary>Recognizes a materialized lexeme collection using this configured parser instance.</summary>\n")
	b.WriteString("    public void ParseInput(IReadOnlyList<Lexeme> input) => _ = ParseValueInput(input);\n")
	b.WriteString("    /// <summary>Recognizes lexemes pulled lazily from a scanner or lexeme source using this configured parser instance.</summary>\n")
	b.WriteString("    public void ParseLexemeSource(ILexemeSource source) => _ = ParseValueLexemeSource(source);\n")
	b.WriteString("    /// <summary>Parses a materialized lexeme collection using this configured parser instance.</summary>\n")
	b.WriteString("    public object? ParseValueInput(IReadOnlyList<Lexeme> input) => ParseValueLexemeSource(new LexemeListSource(input));\n\n")
	b.WriteString("    /// <summary>Parses lexemes pulled lazily from a scanner or lexeme source using this configured parser instance.</summary>\n")
	...
	b.WriteString("    /// <summary>Parses a materialized lexeme collection with grammar-directed recovery using this configured parser instance.</summary>\n")
	b.WriteString("    public ParseResult ParseRecoveringInput(IReadOnlyList<Lexeme> lexemes) => ParseRecoveringLexemeSource(new LexemeListSource(lexemes));\n\n")
	b.WriteString("    /// <summary>Parses lexemes pulled lazily from a scanner or lexeme source with grammar-directed recovery.</summary>\n")
...
func renderSemanticSupport(b *strings.Builder, actions []SemanticAction, semantics spec.SemanticSpec, actionManifest action.Manifest) {
	...
	b.WriteString("/// <summary>Receives target-specific semantic action hooks during parser reductions.</summary>\n")
